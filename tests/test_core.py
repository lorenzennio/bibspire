...
class TestInspireAPI:
    ...
    @patch('time.sleep')
    def test_search_literature_success(self, mock_sleep, mock_get, mock_inspire_response):
...
class TestBibSpire:
    ...
    @patch('builtins.open', new_callable=mock_open)
    def test_update_bib_file(self, mock_file, mock_parse, sample_bib_entry):
        """Test updating BibTeX file"""
        mock_parse.return_value = [sample_bib_entry]
        
        bibspire = BibSpire(delay=0.1, verbose=False)
        with patch.object(bibspire, '_write_bib_file') as mock_write:
            with patch.object(bibspire.inspire, 'search_literature', return_value=None):
