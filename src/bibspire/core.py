...
class BibSpire:
    ...
    def update_bib_file(self, input_file: str, output_file: Optional[str] = None) -> None:
        """
        ...
        for i, entry in enumerate(entries, 1):
            ...
            if inspire_entry:
                print(f"  Updated with INSPIRE data")
    ...
    def _parse_inspire_bibtex(self, bibtex: str, original_key: str) -> Optional[BibEntry]:
        ...
        try:
            ...
            match = re.match(r'@(\w+)\s*\{\s*([^,\s]+)\s*,\s*(.*?)\n\s*\}', 
                           bibtex, re.DOTALL | re.IGNORECASE)
